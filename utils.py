...
class Wave2Mel(object):
    ...
    def __call__(self, x):  #   现在在想，10s的音频利用了多长  经过STFT后的时间步数是大约(L - win_length)/hop_length +1，所以说提取对数梅尔普利用了10s的时间
...
class Wave2spec(object):
    def __init__(self,
        ...
        self.mel_transform = torchaudio.transforms.Spectrogram(win_length=win_length,
                                                                  hop_length=hop_length,
                                                                  n_fft=n_fft,
                                                                  power=power)
        self.amplitude_to_db = torchaudio.transforms.AmplitudeToDB(stype='power')
    ...
    def __call__(self, x):
        # spec =  self.amplitude_to_db(self.mel_transform(x)).squeeze().transpose(-1,-2)
        return self.amplitude_to_db(torch.abs(self.mel_transform(x)))
